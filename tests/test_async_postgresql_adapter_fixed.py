...
class TestAsyncPostgreSQLAdapter(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_execute_select(self, mock_asyncpg):
        ...
        mock_conn = MagicMock()
        mock_conn.fetch = MagicMock(return_value=[
            {"id": 1, "name": "Alice"},
            {"id": 2, "name": "Bob"},
        ])
        ...
        # Create a mock pool
        mock_pool = MagicMock()
        # Make pool.acquire() return a context manager that yields our mock connection
    ...
    async def test_execute_insert(self, mock_asyncpg):
        ...
        # Create a mock connection
        mock_conn = MagicMock()
        mock_conn.execute = MagicMock(return_value="INSERT 0 1")
    ...
    async def test_execute_batch(self, mock_asyncpg):
        ...
        # Create a mock connection
        mock_conn = MagicMock()
        mock_conn.execute = MagicMock(return_value="BATCH EXECUTED")
